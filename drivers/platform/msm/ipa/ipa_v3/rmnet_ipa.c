...
static netdev_tx_t ipa3_wwan_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	trace_ipa_suspend_info("emb-UL", skb->len, ipa3_get_ep_mapping(IPA_CLIENT_APPS_WAN_PROD));
 ...
 * -EINVAL: Invalid args provided
 */
int rmnet_ipa3_remove_stats_counters_from_backup_list(struct ipa_lan_client *lan_client, int device_type)
{
	int ret = 0;
	...
	struct wan_ioctl_lan_client_info *data)
{
	...
	mutex_unlock(&rmnet_ipa3_ctx->per_client_stats_guard);
	rmnet_ipa3_remove_stats_counters_from_backup_list(lan_client, data->device_type);
	return 0;
}
...
int rmnet_ipa3_add_stats_counters_to_backup_list(struct ipa_lan_client *lan_client, int device_type)
{
	...
	memcpy(&client_data.client_info[0].mac, lan_client ->mac, sizeof(lan_client ->mac));
	...
	if(list_empty(&rmnet_ipa3_ctx->msg_stats_backup_cntr_list))
	{
		...
		entry->lan_stats_query.client_info.ipv4_tx_bytes = client_data.client_info[0].ipv4_tx_bytes;
		entry->lan_stats_query.client_info.ipv4_rx_bytes = client_data.client_info[0].ipv4_rx_bytes;
		...
		IPAWANDBG("Client_Id: %d MAC: %02x:%02x:%02x:%02x:%02x:%02x v4_tx_bytes: %lu v4_rx_bytes: %lu v6_tx_bytes: %lu v6_rx_bytes: %lu\n",
 ...
 * -EINVAL: Invalid args provided
 */
int rmnet_ipa3_add_backup_lan_stats_counter(struct wan_ioctl_per_client_info *client_info, int device_type)
{
	int ret = 0;
	struct ipa3_lan_stats_cntr *entry = NULL;
	struct ipa3_lan_stats_cntr *next = NULL;
	...
	if(list_empty(&rmnet_ipa3_ctx->msg_stats_backup_cntr_list))
	{
		IPAWANDBG("entry is empty so no need to add the previous counters\n");
		return ret;
	}
	else
	{
		list_for_each_entry_safe(entry, next,
			...
			if ((entry) && (!memcmp(entry->lan_stats_query.client_info.mac, client_info->mac,
				sizeof(client_info->mac))) && entry->lan_stats_query.device_type == device_type) {
				IPAWANDBG("entry is exists so adding previous counters\n");
				client_info->ipv4_tx_bytes += entry->lan_stats_query.client_info.ipv4_tx_bytes;
...
int ipa3_resend_lan_stats_msg()
{
	...
	else
	{
		list_for_each_entry_safe(entry, next,
			...
			{
				IPAWANDBG("entry is exists\n");
				memcpy(&lan_client,&entry->backup, sizeof(struct wan_ioctl_send_lan_client_msg));
	...
	struct wan_ioctl_send_lan_client_msg *data)
{
	...
	if(data->client_event == IPA_PER_CLIENT_STATS_CONNECT_EVENT)
	{
		if(list_empty(&rmnet_ipa3_ctx->msg_stats_list))
		{
			entry = kzalloc(sizeof(struct ipa3_lan_stats), GFP_KERNEL);
			if(!entry)
			{
				IPAWANERR("failed to alloc memory\n");
				return -EINVAL;
			}
			entry->backup.client_event = IPA_PER_CLIENT_STATS_CONNECT_EVENT;
			memcpy(entry->backup.lan_client.lanIface, data->lan_client.lanIface,
				sizeof(data->lan_client.lanIface));
			memcpy(entry->backup.lan_client.mac, data->lan_client.mac,
				IPA_MAC_ADDR_SIZE* sizeof(uint8_t));
			list_add_tail(&entry->link, &rmnet_ipa3_ctx->msg_stats_list);
		...
		else
		{
			list_for_each_entry_safe(entry, next,
				&rmnet_ipa3_ctx->msg_stats_list, link) {
				/* compare to delete one*/
				if (entry && (!memcmp(entry->backup.lan_client.mac,data->lan_client.mac,
		...
		struct wan_ioctl_query_per_client_stats *data)
{
	...
	for (; i < j; i++) {
		...
		rmnet_ipa3_add_backup_lan_stats_counter(&data->client_info[stats_idx], data->device_type);

		IPAWANDBG("After adding previous counters stats Client ipv4_tx_bytes = %llu, ipv4_rx_bytes = %llu\n",
				data->client_info[stats_idx].ipv4_tx_bytes,
				data->client_info[stats_idx].ipv4_rx_bytes);
		kfree((void *)query->stats);
		ret = result;
