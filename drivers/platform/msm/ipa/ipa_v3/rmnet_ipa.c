...
struct rmnet_ipa3_context {
	...
	struct ipa_sys_connect_params apps_to_ipa_v2x_ep_cfg;
	struct ipa_sys_connect_params ipa_v2x_to_apps_ep_cfg;
	u32 qmap_hdr_hdl;
...
static netdev_tx_t ipa3_wwan_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (ipa_wwan_get_tx_queue(skb) == IPA_RMNET_TX_QUEUE_V2X)
	{
		if (ipa3_ctx->ipa_config_is_auto){
			...
			IPAWANDBG_LOW("Got embedded v2x pkts\n");
			v2x_check = true;
		} else {
			/* in non-AUTO config, 1 for eth traffic */
			IPAWANDBG_LOW("Got embedded eth pdu pkts\n");
			eth_check = true;
		}
	} else {
	...
	if (unlikely(!eth_check) && unlikely(!v2x_check) &&
		...
		/* checking High WM hit for wan traffic only */
 ...
 * @data: data provided with event
 *
 * IPA will pass a v2x packet to the Linux network stack with skb->data
 */
static void apps_ipa_v2x_packet_receive_notify(void *priv,
		enum ipa_dp_evt_type evt,
		unsigned long data)
{
	struct net_device *dev = (struct net_device *)priv;
	...
	if (evt == IPA_RECEIVE) {
		...
		IPAWANDBG_LOW("Rx V2X packet was received");
		skb->dev = IPA_NETDEV();
		if (!rmnet_ipa3_ctx->no_qmap_config)
			skb->protocol = htons(ETH_P_MAP);
		...
		IPAWANERR("Invalid evt %d received in wan_ipa_v2x_receive\n", evt);
	}
}
	...
	struct net_device *dev)
{
	...
	if(!v2x_check){
		...
		ep_idx = ipa_get_ep_mapping(IPA_CLIENT_APPS_WAN_V2X_CONS);
	...
	else if (ingress_param->cs_offload_en) {
		ipa_wan_ep_cfg->ipa_ep_cfg.hdr.hdr_len = 8;
		rmnet_ipa3_ctx->dl_csum_offload_enabled = true;
	} else {
		ipa_wan_ep_cfg->ipa_ep_cfg.hdr.hdr_len = 4;
	...
	if(!v2x_check)
		...
		ipa_wan_ep_cfg->notify = apps_ipa_v2x_packet_receive_notify;
	ipa_wan_ep_cfg->priv = dev;

	if (!v2x_check && ipa3_rmnet_res.ipa_napi_enable)
		ipa_wan_ep_cfg->napi_obj = &(rmnet_ipa3_ctx->wwan_priv->napi);
	ipa_wan_ep_cfg->desc_fifo_sz =
		ipa3_rmnet_res.wan_rx_desc_size * IPA_FIFO_ELEMENT_SIZE;
	...
	bool ip_pdu)
{
	...
	if (ip_pdu) {
		...
		ipa_wan_ep_cfg->ipa_ep_cfg.mode.dst = IPA_CLIENT_APPS_WAN_V2X_PROD;
		ipa_wan_ep_cfg->client = IPA_CLIENT_APPS_WAN_V2X_PROD;
	}
	...
	if (ip_pdu) {
		...
		pipe_status->ep_type = RMNET_EGRESS_V2X_DATA;
		rc = ipa_setup_sys_pipe(
				ipa_wan_ep_cfg, &rmnet_ipa3_ctx->apps_to_ipa3_v2x_hdl);
		if (rc) {
			IPAWANERR("failed to setup egress v2x pipe\n");
			pipe_status->status = IPA_PIPE_SETUP_FAILURE;
			return rc;
		}
...
#endif
{
	...
	case RMNET_IOCTL_EXTENDED:
		...
		/* Get endpoint ID for V2X */
		case RMNET_IOCTL_GET_EPID_V2X:
			IPAWANDBG("get ioctl: RMNET_IOCTL_GET_EPID_V2X\n");
			ext_ioctl_data.u.data = epid_v2x;
			if (copy_to_user((u8 *)ifr->ifr_ifru.ifru_data,
				&ext_ioctl_data,
				sizeof(struct rmnet_ioctl_extended_s)))
				rc = -EFAULT;
			IPAWANDBG("RMNET_IOCTL_GET_EPID_V2X return %d\n",
					ext_ioctl_data.u.data);
			break;

		/*  Endpoint pair for V2X */
		case RMNET_IOCTL_GET_EP_PAIR_V2X:
			IPAWANDBG("get ioctl: RMNET_IOCTL_GET_EP_PAIR_V2X\n");
			wan_ep_v2x =
				ipa_get_ep_mapping(IPA_CLIENT_APPS_WAN_V2X_CONS);
			if (wan_ep_v2x == IPA_EP_NOT_ALLOCATED) {
				IPAWANERR("Embedded datapath not supported\n");
				rc = -EFAULT;
				break;
			}
			...
			wan_ep_v2x =
				ipa_get_ep_mapping(IPA_CLIENT_APPS_WAN_V2X_PROD);
			if (wan_ep_v2x == IPA_EP_NOT_ALLOCATED) {
				IPAWANERR("Embedded datapath not supported\n");
				rc = -EFAULT;
				break;
			...
			if (copy_to_user((u8 *)ifr->ifr_ifru.ifru_data,
				...
				sizeof(struct rmnet_ioctl_extended_s)))
				rc = -EFAULT;
			IPAWANDBG("RMNET_IOCTL_GET_EP_PAIR_V2X c: %d p: %d\n",
			ext_ioctl_data.u.ipa_ep_pair.consumer_pipe_num,
			ext_ioctl_data.u.ipa_ep_pair.producer_pipe_num);
			break;
...
/* Disable NAPI workaround for SA525 */
	if (ipa3_ctx->ipa_config_is_auto){
		ipa_rmnet_drv_res->ipa_napi_enable = false;
		IPADBG("WA to disable NAPI on SA525\n");
	} else {
...
static int ipa3_wwan_probe(struct platform_device *pdev)
{
	...
	memset(&rmnet_ipa3_ctx->apps_to_ipa_ep_eth_cfg, 0,
		sizeof(struct ipa_sys_connect_params));
	memset(&rmnet_ipa3_ctx->apps_to_ipa_v2x_ep_cfg, 0,
		sizeof(struct ipa_sys_connect_params));
	memset(&rmnet_ipa3_ctx->ipa_v2x_to_apps_ep_cfg, 0,
		sizeof(struct ipa_sys_connect_params));
	...
	rmnet_ipa3_ctx->ipa_v2x_set = false;
	for (i = 0; i < MAX_NUM_OF_MUX_CHANNEL; i++)
	{
...
void ipa3_lcl_mdm_reboot_cb ( )
{
	...
	IPAWANDBG("Reboot cb \n");
	if (!rmnet_ipa3_ctx){
		IPAWANERR(" rmnet_ipa3_ctx is not initialised \n");
		return;
	}
	...
	if (ep_idx >= 0) {
		...
		IPAWANDBG("IPA_CLIENT_USB_PROD stopped \n");
	...
	if (ep_idx >= 0) {
		...
		if(res < 0)
			IPAWANDBG("holb enablement is failed on USB CONS\n");
		ep = &ipa3_ctx->ep[ep_idx];
		gsi_stop_channel(ep->gsi_chan_hdl);
	...
	if (ep_idx >= 0) {
		res = ipa3_cfg_ep_holb(ep_idx, &holb_cfg);
		if(res < 0)
			IPAWANDBG("holb enablement is failed on WLAN2 CONS\n");
		ep = &ipa3_ctx->ep[ep_idx];
		gsi_stop_channel(ep->gsi_chan_hdl);
	...
	if (ep_idx >= 0) {
		res = ipa3_cfg_ep_holb(ep_idx, &holb_cfg);
		if(res < 0)
			IPAWANDBG("holb enablement is failed on WLAN2 CONS1\n");
		ep = &ipa3_ctx->ep[ep_idx];
		gsi_stop_channel(ep->gsi_chan_hdl);
	...
	if (ep_idx >= 0) {
		res = ipa3_cfg_ep_holb(ep_idx, &holb_cfg);
		if(res < 0)
			IPAWANDBG("holb enablement is failed on APPS WAN CONS\n");
		ep = &ipa3_ctx->ep[ep_idx];
		gsi_stop_channel(ep->gsi_chan_hdl);
	...
	if (ep_idx >= 0) {
		res = ipa3_cfg_ep_holb(ep_idx, &holb_cfg);
		if(res < 0)
			IPAWANDBG("holb enablement is failed on LAN CONS\n");
		ep = &ipa3_ctx->ep[ep_idx];
		gsi_stop_channel(ep->gsi_chan_hdl);
	...
	if (ep_idx >= 0) {
		res = ipa3_cfg_ep_holb(ep_idx, &holb_cfg);
		if(res < 0)
			IPAWANDBG("holb enablement is failed on ETHERNET CONS\n");
		ep = &ipa3_ctx->ep[ep_idx];
		gsi_stop_channel(ep->gsi_chan_hdl);
			   ...
			   void *data)
{
	...
	case SUBSYS_BEFORE_DS_EXIT:
		...
		if (ipa3_ctx->ipa_config_is_mhi) {
			...
			ipa3_set_reset_client_cons_pipe_sus_holb(false, IPA_CLIENT_MHI_LOW_LAT_CONS);
			if (ipa3_ctx->ipa_hw_type >= IPA_HW_v5_5)
				ipa3_set_reset_client_cons_pipe_sus_holb(false, IPA_CLIENT_MHI_COAL_CONS);
		}
...
static inline bool rmnet_ipa3_check_any_client_inited_v2
(
	enum ipacm_per_client_device_type device_type
)
{
	int i = 0;
	struct ipa_tether_device_info_v2 *teth_v2 = NULL;

	for (; i < IPA_MAX_NUM_HW_PATH_CLIENTS_V2; i++)	{
		teth_v2 = &rmnet_ipa3_ctx->tether_device_v2[device_type];

		if (teth_v2->lan_client[i].client_idx != -1 &&
			teth_v2->lan_client[i].inited) {
			IPAWANERR("Found client index: %d which is inited\n", i);
			return true;
		}
	}
...
static inline int rmnet_ipa3_get_lan_client_info_v2(
	enum ipacm_per_client_device_type device_type,
	uint8_t mac[])
{
	int i = 0;
	struct ipa_tether_device_info_v2 *teth_ptr_v2 = NULL;
	...
	for (; i < IPA_MAX_NUM_HW_PATH_CLIENTS_V2; i++) {
		teth_ptr_v2 = &rmnet_ipa3_ctx->tether_device_v2[device_type];
		if (memcmp(
			teth_ptr_v2->lan_client[i].mac,
...
static inline int rmnet_ipa3_delete_lan_client_info_v2
(
	enum ipacm_per_client_device_type device_type,
	int lan_clnt_idx
)
{
	struct ipa_lan_client *lan_client_v2 = NULL;
	int i;
	struct ipa_tether_device_info_v2 *teth_ptr_v2 = NULL;
	...
	teth_ptr_v2 = &rmnet_ipa3_ctx->tether_device_v2[device_type];
	IPAWANDBG("Delete lan client info v2: %d, %d, %d (v2)\n",
		teth_ptr_v2->num_clients, lan_clnt_idx, device_type);
	/* Check if the request is to clean up all clients. */
	if (lan_clnt_idx == 0xffffffff) {
		/* Reset the complete device info. */
		memset(teth_ptr_v2, 0,
			sizeof(struct ipa_tether_device_info_v2));
		teth_ptr_v2->ul_src_pipe = -1;
		for (i = 0; i < IPA_MAX_NUM_HW_PATH_CLIENTS_V2; i++)
			teth_ptr_v2->lan_client[i].client_idx = -1;
	} else {
		lan_client_v2 = &teth_ptr_v2->lan_client[lan_clnt_idx];
		/* Reset the client info before sending the message. */
		memset(lan_client_v2, 0, sizeof(struct ipa_lan_client));
...
static int rmnet_ipa_get_hw_fnr_stats_v3(
	struct ipa_lan_wan_client_cntr_index *client,
	struct wan_ioctl_query_per_client_stats *data,
	struct ipa_ioc_flt_rt_query *query, bool query_flt, bool query_rt)
{
	int num_counters;
...
/* rmnet_ipa3_clear_lan_client_info() -
 * @data - IOCTL data
 *
...
/* rmnet_ipa3_set_lan_client_info_v2() -
 * @data - IOCTL data
 *
 * This function handles WAN_IOC_SET_LAN_TO_LAN_CLIENT_INFO.
 * It is used to store LAN client information which
 * is used to fetch the packet stats for a client.
 *
 * Return codes:
 ...
 * -EINVAL: Invalid args provided
 */
int rmnet_ipa3_set_lan_client_info_v2(
	struct wan_ioctl_lan_client_info_v2 *data)
{
	struct ipa_lan_client *lan_client = NULL;
	struct ipa_lan_wan_client_cntr_index
		*client_index = NULL;
	struct ipa_tether_device_info_v2 *teth_ptr = NULL;
	...
	/* This should be done when allocation of hw fnr counters happens */
	if (!(data->wan_cnt_idx > 0 &&
		data->lan_cnt_idx == (data->wan_cnt_idx + 1))) {
		IPAWANERR("Invalid counter indices %u, %u\n",
				data->wan_cnt_idx, data->lan_cnt_idx);
		return -EINVAL;
	}
	...
	if (data->client_init) {
		/* check if the client is already inited. */
		if (rmnet_ipa3_ctx->tether_device_v2[data->device_type]
			.lan_client[data->client_idx].inited) {
			IPAWANERR("Client already inited: %d:%d\n",
				data->device_type, data->client_idx);
			mutex_unlock(&rmnet_ipa3_ctx->per_client_stats_guard);
	...
	teth_ptr = &rmnet_ipa3_ctx->tether_device_v2[data->device_type];
	lan_client = &teth_ptr->lan_client[data->client_idx];
	client_index = &teth_ptr->lan_wan_client_indices[data->client_idx];
	...
	/* Update the Source pipe. */
	rmnet_ipa3_ctx->tether_device_v2[data->device_type].ul_src_pipe =
			ipa_get_ep_mapping(data->ul_src_pipe);

	/* Update the header length if not set. */
	if (!rmnet_ipa3_ctx->tether_device_v2[data->device_type].hdr_len)
		rmnet_ipa3_ctx->tether_device_v2[data->device_type].hdr_len =
			data->hdr_len;
	client_index->wan_cnt_idx = data->wan_cnt_idx;
	client_index->lan_cnt_idx = data->lan_cnt_idx;
	...
	rmnet_ipa3_ctx->tether_device_v2[data->device_type].num_clients++;
...
/* rmnet_ipa3_clear_lan_client_info_v2() -
 * @data - IOCTL data
 *
 * This function handles WAN_IOC_DELETE_LAN_TO_LAN_CLIENT_INFO.
 * It is used to delete LAN client information which
 * is used to fetch the packet stats for a client.
 *
 * Return codes:
 ...
 * -EINVAL: Invalid args provided
 */
int rmnet_ipa3_clear_lan_client_info_v2(
	struct wan_ioctl_lan_client_info_v2 *data)
{
	struct ipa_lan_client *lan_client = NULL;
	struct ipa_tether_device_info_v2 *teth_ptr = NULL;
		...
		struct wan_ioctl_query_per_client_stats *data)
{
	...
	/* Check if Source pipe is valid. */
	if (rmnet_ipa3_ctx->tether_device_v2
			[data->device_type].ul_src_pipe == -1) {
		IPAWANERR("Device not initialized: %d\n", data->device_type);
		mutex_unlock(&rmnet_ipa3_ctx->per_client_stats_guard);
		return -EINVAL;
	...
	/* Check if we have clients connected. */
	if (rmnet_ipa3_ctx->tether_device_v2[data->device_type].num_clients == 0) {
		IPAWANERR("No clients connected: %d\n", data->device_type);
		mutex_unlock(&rmnet_ipa3_ctx->per_client_stats_guard);
		return -EINVAL;
	}
	...
	if (data->num_clients == 1) {
		/* Check if the client info is valid.*/
		lan_clnt_idx = rmnet_ipa3_get_lan_client_info_v2(
				data->device_type,
				data->client_info[0].mac);
		if (lan_clnt_idx < 0) {
			IPAWANERR("Client info not available return.\n");
		...
		struct wan_ioctl_query_per_client_stats_v2 *data)
{
	...
	if (data->num_clients == 1) {
		...
		if (data->stats_type == LAN2LAN_STATS) {
			lan_clnt_idx = rmnet_ipa3_get_lan_client_info_v2(
				data->device_type,
				data->lan2lan_client_info[0].mac);
			if (lan_clnt_idx < 0) {
				IPAWANERR("Client info not available return.\n");
				kfree((void *)query->stats);
				mutex_unlock(&rmnet_ipa3_ctx->per_client_stats_guard);
	...
	for (; i < j; i++) {
		...
		if (data->stats_type == LAN2LAN_STATS) {
			...
			data->lan2lan_client_info[stats_idx].ipv4_rx_bytes =
				fnr_stats->num_bytes;

			memcpy(data->lan2lan_client_info[stats_idx].mac,
				lan_client[i].mac, IPA_MAC_ADDR_SIZE);
...
int ipa3_wwan_init(void)
{
	...
	/* Reset the Lan Stats v2 */
	if (ipa3_ctx->ipa_hw_type >= IPA_HW_v6_0 && !ipa3_ctx->ipa_config_is_auto) {
		/* Reset the Lan Stats v2. */
		for (i = 0; i < IPACM_MAX_CLIENT_DEVICE_TYPES; i++) {
			teth_ptr_v2 = &rmnet_ipa3_ctx->tether_device_v2[i];
			teth_ptr_v2->ul_src_pipe = -1;

			for (j = 0; j < GET_MAX_HW_PATH_CLIENTS(); j++)
				teth_ptr_v2->lan_client[j].client_idx = -1;
		}
	} else {
		/* Reset the Lan Stats. */
		for (i = 0; i < IPACM_MAX_CLIENT_DEVICE_TYPES; i++) {
