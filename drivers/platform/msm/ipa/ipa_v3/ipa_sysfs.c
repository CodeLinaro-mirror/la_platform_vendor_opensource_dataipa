...
extern const char *ipa3_hdr_l2_type_name[];
...
static ssize_t holb_monitor_client_add_del_store(struct device *dev, struct device_attribute *attr, const char *ubuf, size_t count)
{
...
static ssize_t ep_reg_show(struct device *dev, struct device_attribute *attr, char *ubuf)
{
	...
	for (i = start_idx; i < end_idx; i++) {
		...
		nbytes = ipa3_ctx->ctrl->ipa3_read_ep_reg(dbg_buff,
				IPA_MAX_MSG_LEN, i);
		ret = memcpy(ubuf, dbg_buff, nbytes);
		if (NULL == ret) {
			IPA_ACTIVE_CLIENTS_DEC_SIMPLE();
			return nbytes;
		}
...
static ssize_t set_clk_idx_store(struct device *dev, struct device_attribute *attr, const char *ubuf, size_t count)
{
	s8 option = 0;
	int ret;
	uint32_t bw_idx = 0;
...
static ssize_t mpm_ring_size_dl_show(struct device *dev, struct device_attribute *attr, char *ubuf)
{
	int nbytes;
...
static ssize_t mpm_ring_size_ul_show(struct device *dev, struct device_attribute *attr, char *ubuf)
{
	int nbytes;
...
static ssize_t mpm_uc_thresh_show(struct device *dev, struct device_attribute *attr, char *ubuf)
{
	int nbytes;
...
static ssize_t mpm_teth_aggr_size_show(struct device *dev, struct device_attribute *attr, char *ubuf)
{
	int nbytes;
...
static ssize_t mpm_ring_size_dl_store(struct device *dev, struct device_attribute *attr, const char *ubuf, size_t count)
{
	s8 option = 0;
	int ret;
...
static ssize_t mpm_ring_size_ul_store(struct device *dev, struct device_attribute *attr, const char *ubuf, size_t count)
{
	s8 option = 0;
	int ret;
...
static ssize_t mpm_uc_thresh_store(struct device *dev, struct device_attribute *attr, const char *ubuf, size_t count)
{
	s8 option = 0;
	int ret;
...
static ssize_t mpm_teth_aggr_size_store(struct device *dev, struct device_attribute *attr, const char *ubuf, size_t count)
{
	s8 option = 0;
	int ret;
...
static ssize_t holb_events_show(struct device *dev, struct device_attribute *attr, char *ubuf)
{
	int nbytes = 0;
	int client_idx;
	int event_id;
...
static ssize_t lan_coal_stats_show(struct device *dev, struct device_attribute *attr, char *ubuf)
{
	int nbytes=0, cnt=0;
	u32 i;
	char buf[1024];
...
static ssize_t cache_recycle_stats_show(struct device *dev, struct device_attribute *attr, char *ubuf)
{
	int nbytes;
	int cnt = 0;
...
static ssize_t ipa_max_napi_sort_page_thrshld_show(struct device *dev, struct device_attribute *attr, char *ubuf) {
...
static ssize_t ipa_dscp_pcp_mapping_cache_show(struct device *dev, struct device_attribute *attr, char *ubuf) {
...
static ssize_t ipa_max_napi_sort_page_thrshld_store(struct device *dev, struct device_attribute *attr, const char *ubuf, size_t count) {
	...
	ret = kstrtou8(ubuf, 0, &ipa_max_napi_sort_page_thrshld);
	if(ret != 0)
		return ret;
...
static ssize_t page_wq_reschd_time_show(struct device *dev, struct device_attribute *attr, char *ubuf)
{
	int nbytes;
	nbytes = scnprintf(dbg_buff, IPA_MAX_MSG_LEN,
...
static ssize_t page_wq_reschd_time_store(struct device *dev, struct device_attribute *attr, const char *ubuf, size_t count)
{
	int ret;
	...
	ret = kstrtou8(ubuf, 0, &page_wq_reschd_time);
	if(ret != 0)
		return ret;
...
static ssize_t page_poll_threshold_show(struct device *dev, struct device_attribute *attr, char *ubuf)
{
	int nbytes;
	nbytes = scnprintf(dbg_buff, IPA_MAX_MSG_LEN,
...
static ssize_t page_poll_threshold_store(struct device *dev, struct device_attribute *attr, const char *ubuf, size_t count)
{
	int ret;
	...
	ret = kstrtou8(ubuf,  0, &page_poll_threshold);
	if(ret != 0)
		return ret;
...
static ssize_t move_nat_table_to_ddr_store(struct device *dev, struct device_attribute *attr, const char *ubuf, size_t count)
{
	u32 direction;
	char *sptr, *token;
...
#if defined(CONFIG_IPA_TSP)
static ssize_t tsp_show(struct device *dev, struct device_attribute *attr, char *ubuf)
{
	int i, nbytes = 0;
...
static ssize_t tsp_store(struct device *dev, struct device_attribute *attr, const char *ubuf, size_t count) {
...
static ssize_t ntn3_0_err_status_show(struct device *dev, struct device_attribute *attr, char *ubuf)
{
	return __eth_err_status_show(IPA_ETH_CLIENT_NTN3, 0, ubuf);
}

static ssize_t ntn3_1_err_status_show(struct device *dev, struct device_attribute *attr, char *ubuf)
{
	return __eth_err_status_show(IPA_ETH_CLIENT_NTN3, 1, ubuf);
}

static ssize_t iemac_0_err_status_show(struct device *dev, struct device_attribute *attr, char *ubuf)
{
	return __eth_err_status_show(IPA_ETH_CLIENT_IEMAC, 0, ubuf);
}

static ssize_t iemac_1_err_status_show(struct device *dev, struct device_attribute *attr, char *ubuf)
{
...
static DEVICE_ATTR_RO(cache_recycle_stats);
...
static DEVICE_ATTR_RO(ipa_dscp_pcp_mapping_cache);

#if defined(CONFIG_IPA_IPSEC)
static DEVICE_ATTR_RO(ipsec_encap_sa_info);
static DEVICE_ATTR_RO(ipsec_decap_sa_info);
...
static DEVICE_ATTR_RO(ntn3_0_err_status);
...
static DEVICE_ATTR_WO(move_nat_table_to_ddr);
static DEVICE_ATTR_WO(set_clk_idx);
#if defined(CONFIG_IPA_IPSEC)
static DEVICE_ATTR_WO(ipsec_set_sa_info_index);
#endif
...
static DEVICE_ATTR_RW(mpm_ring_size_ul);
static DEVICE_ATTR_RW(mpm_uc_thresh);
static DEVICE_ATTR_RW(mpm_teth_aggr_size);

static DEVICE_ATTR_RW(dbg_cnt);
static DEVICE_ATTR_RW(page_poll_threshold);
static DEVICE_ATTR_RW(page_wq_reschd_time);
static DEVICE_ATTR_RW(ipa_max_napi_sort_page_thrshld);
#if defined(CONFIG_IPA_TSP)
static DEVICE_ATTR_RW(tsp);
#endif
...
static struct attribute *ipa_attrs[] = {
	...
	&dev_attr_mpm_ring_size_dl.attr,
	&dev_attr_mpm_ring_size_ul.attr,
	&dev_attr_mpm_uc_thresh.attr,
	&dev_attr_mpm_teth_aggr_size.attr,
	&dev_attr_set_clk_idx.attr,
	&dev_attr_holb.attr,
	&dev_attr_holb_monitor_client_param.attr,
	&dev_attr_holb_monitor_client_add_del.attr,
	&dev_attr_holb_events.attr,
	&dev_attr_hdr.attr,
	...
	&dev_attr_cache_recycle_stats.attr,
	&dev_attr_wdi.attr,
	&dev_attr_ntn.attr,
	...
	&dev_attr_page_poll_threshold.attr,
	&dev_attr_move_nat_table_to_ddr.attr,
	...
	&dev_attr_ntn3_0_err_status.attr,
...
#endif
#if defined(CONFIG_IPA_IPSEC)
	&dev_attr_ipsec_set_sa_info_index.attr,
	&dev_attr_ipsec_encap_sa_info.attr,
	&dev_attr_ipsec_decap_sa_info.attr,
	&dev_attr_ipsec_active_sa.attr,
#endif
	NULL
};
