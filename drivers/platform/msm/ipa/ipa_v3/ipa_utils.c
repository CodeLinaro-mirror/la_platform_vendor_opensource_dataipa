...
/* Only for test purpose */
	...
	[IPA_5_2_MDM][IPA_CLIENT_APPS_WAN_ETH_PROD] = {
			true, IPA_v5_2_GROUP_UL,
			true,
			IPA_DPS_HPS_SEQ_TYPE_2ND_PKT_PROCESS_PASS_NO_DEC_UCP,
			QMB_MASTER_SELECT_DDR,
...
static struct ipa3_mem_partition ipa_5_2_mdm_eth_pdu_mem_part = {
	.uc_descriptor_ram_ofst = 0x0,
	.uc_descriptor_ram_size = 0x600,
	.uc_ofst = 0x600,
	.uc_size = 0x80,
...
int ipa3_init_mem_partition(enum ipa_hw_type type)
{
	...
	case IPA_HW_v5_2:
		if (ipa3_ctx->platform_type == IPA_PLAT_TYPE_MDM) {
			if (ipa3_ctx->ipa_eth_pdu_enable)
				ipa3_ctx->ctrl->mem_partition = &ipa_5_2_mdm_eth_pdu_mem_part;
			else
