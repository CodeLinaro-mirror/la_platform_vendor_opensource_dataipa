...
const char * const ipa_rc_client_names[MAX_RC_CLIENTS] = {
	"ETH", "WLAN", "MODEM", "ETH1", "WLAN2", "WLAN3", "OTHERS"
};
...
static const struct {
	...
	{ IPA_CHANNEL_ETH1_NOT_STARTED,             "IPA_CHANNEL_ETH1_NOT_STARTED"             },
	{ IPA_CHANNEL_WLAN2_NOT_STARTED,            "IPA_CHANNEL_WLAN2_NOT_STARTED"            },
	{ IPA_CHANNEL_WLAN3_NOT_STARTED,            "IPA_CHANNEL_WLAN3_NOT_STARTED"            },
	{ IPA_CHANNEL_ETH1_NO_BUFF,                 "IPA_CHANNEL_ETH1_NO_BUFF"                 },
	{ IPA_CHANNEL_WLAN2_NO_BUFF,                "IPA_CHANNEL_WLAN2_NO_BUFF"                },
	{ IPA_CHANNEL_WLAN3_NO_BUFF,                "IPA_CHANNEL_WLAN3_NO_BUFF"                },
	{ IPA_DRIVER_ETH1_PKT_DROP,                 "IPA_DRIVER_ETH1_PKT_DROP"                 },
	{ IPA_DRIVER_WLAN2_AP_PKT_DROP,             "IPA_DRIVER_WLAN2_AP_PKT_DROP"             },
	{ IPA_DRIVER_WLAN2_STA_PKT_DROP,            "IPA_DRIVER_WLAN2_STA_PKT_DROP"            },
	{ IPA_DRIVER_WLAN3_AP_PKT_DROP,             "IPA_DRIVER_WLAN3_AP_PKT_DROP"             },
	{ IPA_DRIVER_WLAN3_STA_PKT_DROP,            "IPA_DRIVER_WLAN3_STA_PKT_DROP"            },
	{ IPA_ETH1_FILTER_RULE_INCORRECT,           "IPA_ETH1_FILTER_RULE_INCORRECT"           },
	{ IPA_WLAN2_AP_FILTER_RULE_INCORRECT,       "IPA_WLAN2_AP_FILTER_RULE_INCORRECT"       },
	{ IPA_WLAN2_STA_FILTER_RULE_INCORRECT,      "IPA_WLAN2_STA_FILTER_RULE_INCORRECT"      },
	{ IPA_WLAN3_AP_FILTER_RULE_INCORRECT,       "IPA_WLAN3_AP_FILTER_RULE_INCORRECT"       },
	{ IPA_WLAN3_STA_FILTER_RULE_INCORRECT,      "IPA_WLAN3_STA_FILTER_RULE_INCORRECT"      },
	{ IPA_DRIVER_MODEM_PKT_DROP,                "IPA_DRIVER_MODEM_PKT_DROP"                },
};
...
bool is_wlan_sta_pkt(struct ipahal_pkt_status *status)
{
	...
	if(!is_any_wlan_prod_pipe(ipa3_get_client_by_pipe(i))) {
		if(ipa3_ctx->is_rc_log_enabled)
			IPADBG("src: %d not wlan pipe\n", status->endp_src_idx);

		return false;
	}

	if(ipa3_ctx->is_rc_log_enabled)
		IPADBG("status metadata: %d\n", ntohl(status->metadata));
	...
	for (i = 0; i < MAX_RC_CLIENTS; i++) {
		for (dir = 0; dir < DATA_DIR; dir++) {
			...
			if (i == WLAN)
				...
				bit = (dir == 0) ? IPA_DRIVER_WLAN2_AP_PKT_DROP :
						   IPA_DRIVER_WLAN2_STA_PKT_DROP;
			else if (i == WLAN3)
				bit = (dir == 0) ? IPA_DRIVER_WLAN3_AP_PKT_DROP :
						   IPA_DRIVER_WLAN3_STA_PKT_DROP;
			else if (i == ETH)
				bit = IPA_DRIVER_ETH_PKT_DROP;
			else if (i == ETH1)
				bit = IPA_DRIVER_ETH1_PKT_DROP;
			else if (i == MODEM)
				bit = IPA_DRIVER_MODEM_PKT_DROP;
			else
				continue;
...
void ipa_rc_reset_drop_pkt_stats(void)
{
	...
	for (i = 0; i < MAX_RC_CLIENTS; i++) {
		ipa3_ctx->stats.rx_excp_pkts[i][IPAHAL_PKT_STATUS_EXCEPTION_DROP_UL] = 0;
		ipa3_ctx->stats.rx_excp_pkts[i][IPAHAL_PKT_STATUS_EXCEPTION_DROP_DL] = 0;
	}
...
void ipa_rc_query_chan(enum ipa_client_type clnt, int chan, struct chan_param_monitor* chan_params)
{
	...
	if (ee < 0) {
		IPADBG("Failed to get EE for client %d\n", clnt);
		return;
	}
			...
			struct chan_param_monitor* chan_params)
{
	...
	if(!chan_params) {
		IPADBG("chan param NULL\n");
		return cur_status;
	}

	if (client < 0 || client >= IPA_CLIENT_MAX) {
		IPADBG("invalid client type %d\n", client);
		return cur_status;
	}
...
void ipa_rc_query_detect_chan_n(struct ipa_rc_health_monitor *ipa_state_info)
{
	...
	for (i = 1; i < IPA_CLIENT_MAX; i+=2) {
		idx = ipa_get_ep_mapping(i);
		...
		if(is_any_eth_cons_pipe(i) ||
			is_any_wlan_cons_pipe(i)) {
			if(ipa3_find_intf_by_client(i) != 0) {
				if(ipa3_ctx->is_rc_log_enabled)
					IPADBG("ep %d not configured\n", idx);
				continue;
			}
		}
		else if(!IPA_CLIENT_IS_Q6_CONS(i) && !ipa3_ctx->ep[idx].valid) {
			if(ipa3_ctx->is_rc_log_enabled)
				IPADBG("ep %d not configured\n", idx);
			continue;
		}
		...
		if(is_any_eth_cons_pipe(i) ||
			is_any_wlan_cons_pipe(i) ||
			IPA_CLIENT_IS_APPS_CONS(i) ||
			IPA_CLIENT_IS_Q6_CONS(i))
		{
			chan = ipa3_get_chan_by_client(i);
			memset(&chan_params, 0, sizeof(chan_params));
			ipa_rc_query_chan(i, chan, &chan_params);
			status |= ipa_rc_detect_chan_stall(i, &chan_params);
		}
	}
						...
						enum ipa_ip_type ip, uint32_t* metadata, int *has_sta_rule)
{
	...
	list_for_each_entry(entry, &tbl->head_flt_rule_list, link) {
		...
		if((grp_id == WLAN_STA_DL_FLT_RULE || grp_id == ETH_DL_FLT_RULE) && has_sta_rule)
			*has_sta_rule = 1;
...
void ipa_rc_nat_init(struct ipa_rc_health_monitor *ipa_state_info)
{
	...
	if (!ndev->is_dev_init || !ndev->is_hw_init || !any_table_active) {
		if(ipa3_ctx->is_rc_log_enabled)
			IPADBG("NAT hasn't been initialized\n");
