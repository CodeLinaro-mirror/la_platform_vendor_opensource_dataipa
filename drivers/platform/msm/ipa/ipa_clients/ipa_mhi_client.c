...
#define IPA_MHI_DBG(fmt, args...) \
	do { \
		...
		IPA_IPC_LOGGING(ipa3_get_ipc_logbuf_low(), \
			IPA_MHI_DRV_NAME " %s:%d " fmt, ## args); \
		ipa3_diag_log_write(IPA_DIAG_LVL_DBG, IPA_MHI_DRV_NAME " %s:%d " fmt, \
			__func__, __LINE__, ## args); \
	} while (0)
...
#define IPA_MHI_ERR(fmt, args...) \
	do { \
		...
		IPA_IPC_LOGGING(ipa3_get_ipc_logbuf_low(), \
				IPA_MHI_DRV_NAME " %s:%d " fmt, ## args); \
		ipa3_diag_log_write(IPA_DIAG_LVL_ERR, IPA_MHI_DRV_NAME " %s:%d " fmt, \
			__func__, __LINE__, ## args); \
	} while (0)
		...
		struct ipa_mhi_channel_ctx *channels, int max_channels)
{
	...
	for (i = 0; i < max_channels; i++) {
		...
		case IPA_MHI_STATE_M0:
			if (ipa_mhi_client_ctx->state != IPA_MHI_STATE_SUSPEND_IN_PROGRESS) {
...
int ipa_mhi_client_get_ep_mapping(u8 channel_id)
{
	...
	IPA_MHI_DBG("channel_id = %d clnt_hdl = %d client = %s\n",
			channel_id, clnt_hdl, ipa_clients_strings[client]);
	return clnt_hdl;
}
