...
static void __gsi_read_ntn_channel_scratch2_reg(unsigned long chan_hdl,
...
static void __gsi_write_ntn_channel_scratch2_reg(unsigned long chan_hdl,
				union __packed gsi_ntn_channel_scratch2_reg val)
{
	gsihal_write_reg_nk(GSI_EE_n_GSI_CH_k_SCRATCH_2,
		gsi_ctx->per.ee, chan_hdl, val.data.word1);
		...
		union __packed gsi_ntn_channel_scratch2_reg * val)
{
	...
	mutex_lock(&ctx->mlock);
	__gsi_read_ntn_channel_scratch2_reg(chan_hdl, val);
	mutex_unlock(&ctx->mlock);

	return GSI_STATUS_SUCCESS;
}
EXPORT_SYMBOL(gsi_read_ntn_channel_scratch2_reg);
		...
		union __packed gsi_ntn_channel_scratch2_reg val)
{
	...
	mutex_lock(&ctx->mlock);
	__gsi_write_ntn_channel_scratch2_reg(chan_hdl, val);
	mutex_unlock(&ctx->mlock);

	return GSI_STATUS_SUCCESS;
}
EXPORT_SYMBOL(gsi_write_ntn_channel_scratch2_reg);
...
int gsi_start_channel(unsigned long chan_hdl)
{
	...
	if (ctx->state != GSI_CHAN_STATE_ALLOCATED &&
		...
		GSIERR("bad state %d for channel hdl %lu\n", ctx->state, chan_hdl);
		return -GSI_STATUS_UNSUPPORTED_OP;
	}
